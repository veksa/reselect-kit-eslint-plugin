...
export const getSelectorProps = (selectorType: ts.Type, typeChecker: ts.TypeChecker) => {
  const [signature] = typeChecker.getSignaturesOfType(selectorType, ts.SignatureKind.Call);
  ...
  const nodeType = typeChecker.getTypeOfSymbolAtLocation(props, props.valueDeclaration);
